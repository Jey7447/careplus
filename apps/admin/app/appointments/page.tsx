...
export default async function AppointmentsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [{ data: patients }, { data: doctors }] = await Promise.all([
    ...
    doctorIds.length
      ? supabase.from('doctors').select('doctor_id, first_name, last_name, specialty').in('doctor_id', doctorIds)
      : Promise.resolve({ data: [], error: null }),
  ]);
            ...
            <section className="rounded-2xl border border-[var(--border)] bg-white p-6">
              <form className="grid gap-4 lg:grid-cols-[1fr_220px_auto]" method="get">
                <label>
                  ...
                  <input name="search" defaultValue={search} placeholder="Patient, doctor, type or reason" className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-200" />
                </label>
                <label>
                  <span className="text-sm font-medium text-slate-700">Status</span>
                  <select name="status" defaultValue={status} className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-200">
            ...
            <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white">
              <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">Showing {filteredRows.length} of up to 100 loaded appointments.</p>
                </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredRows.map((appointment) => (
                      <tr key={appointment.appointment_id} className="hover:bg-slate-50">
                        <td className="whitespace-nowrap px-6 py-4 font-medium text-slate-900">#{appointment.appointment_id}</td>
                        <td className="px-6 py-4"><p className="font-medium text-slate-900">{patientMap.get(appointment.patient_id) ?? 'Unknown patient'}</p><p className="mt-1 text-xs text-slate-500">{appointment.reason_for_visit ?? 'No reason recorded'}</p></td>
                        <td className="px-6 py-4"><p className="font-medium text-slate-900">{doctorMap.get(appointment.doctor_id) ?? 'Unknown doctor'}</p><p className="mt-1 text-xs text-slate-500">{specialtyMap.get(appointment.doctor_id) ?? ''}</p></td>
                        <td className="whitespace-nowrap px-6 py-4"><p className="font-medium text-slate-900">{formatDate(appointment.appointment_date)}</p><p className="mt-1 text-xs text-slate-500">{formatTime(appointment.appointment_time)}</p></td>
                        <td className="whitespace-nowrap px-6 py-4 text-slate-600">{appointment.appointment_type}</td>
                        <td className="whitespace-nowrap px-6 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusClass(appointment.appointment_status)}`}>{appointment.appointment_status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
