    ...
    <main className="min-h-screen"><div className="flex min-h-screen">
      ...
      <section className="flex-1"><header className="border-b border-[var(--border)] bg-white px-6 py-5 lg:px-8"><div className="mx-auto flex max-w-7xl items-center justify-between gap-6"><div><p className="text-sm text-slate-500">CarePlus Medical Centre</p><h1 className="mt-1 text-2xl font-semibold">Dashboard</h1><p className="mt-1 text-sm text-slate-500">Overview of centre activity.</p></div><div className="hidden text-right sm:block"><p className="text-xs text-slate-500">Signed in as</p><p className="mt-1 text-sm font-medium text-slate-800">{email}</p></div></div></header>
        <div className="mx-auto max-w-7xl space-y-6 p-6 lg:p-8"><div className="rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm text-slate-600">Authentication, administrator authorization, and protected database access are active.</div>{dataErrors.length > 0 && <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-800">Some dashboard data could not be loaded. The protected connection is active, but one or more database queries returned an error.</div>}
          ...
          <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]"><section className="rounded-2xl border border-[var(--border)] bg-white p-6"><div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold">Upcoming appointments</h2><p className="mt-1 text-sm text-slate-500">Scheduled and confirmed appointments from CarePlus.</p></div><Link href="/appointments" className="text-sm font-medium text-slate-700 hover:text-slate-900">View all</Link></div><div className="mt-6 overflow-hidden rounded-xl border border-slate-200">{upcoming.length === 0 ? <div className="p-10 text-center text-sm text-slate-500">No upcoming appointments found.</div> : <div className="divide-y divide-slate-100">{upcoming.map((appointment) => <Link key={appointment.appointment_id} href={`/appointments/${appointment.appointment_id}`} className="flex flex-col gap-3 px-4 py-4 transition-colors hover:bg-slate-50 sm:flex-row sm:items-center sm:justify-between outline-none focus:bg-slate-50 focus:outline-none focus-visible:bg-slate-50"><div><div className="flex items-center gap-2"><span className="text-xs font-semibold text-slate-500">Appointment #{appointment.appointment_id}</span><span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${appointment.appointment_status === 'Confirmed' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}>{appointment.appointment_status}</span></div><p className="mt-1 text-sm font-medium text-slate-900">{patientMap.get(appointment.patient_id) ?? 'Patient'}</p><p className="mt-1 text-xs text-slate-500">{doctorMap.get(appointment.doctor_id) ?? 'Doctor'}</p></div><div className="text-left sm:text-right"><p className="text-sm font-medium text-slate-800">{formatDate(appointment.appointment_date)}</p><p className="mt-1 text-xs text-slate-500">{formatTime(appointment.appointment_time)}</p><p className="mt-1 text-xs font-medium text-slate-400">Open appointment →</p></div></Link>)}</div>}</div></section><section className="rounded-2xl border border-[var(--border)] bg-white p-6"><h2 className="font-semibold">System status</h2><div className="mt-5 space-y-4 text-sm">{[['Supabase database', 'Connected'], ['Appointment automation', 'Active'], ['Notification tracking', 'Active']].map(([item, status]) => <div key={item} className="flex justify-between gap-4"><span className="text-slate-600">{item}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{status}</span></div>)}</div></section></div>
        </div></section>
    </div></main>
  );
}
