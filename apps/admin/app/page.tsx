...
export default async function Home() {
  ...
  const stats = [
    ["Today's Appointments", appointmentsCount.count ?? 0, CalendarDays],
    ['Total Patients', patientsCount.count ?? 0, Users],
    ['Active Doctors', doctorsCount.count ?? 0, Stethoscope],
    ['Pending Notifications', notificationsCount.count ?? 0, ClipboardList],
  ] as const;
        ...
        <section className="flex-1">
          ...
          <div className="mx-auto max-w-7xl space-y-6 p-6 lg:p-8">
            <div className="rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm text-slate-600">
              Authentication, administrator authorization, and protected database access are active.
            </div>

            {dataErrors.length > 0 && (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-800">
                Some dashboard data could not be loaded. The protected connection is active, but one or more database queries returned an error.
              </div>
            )}
            ...
            <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
              <section className="rounded-2xl border border-[var(--border)] bg-white p-6">
                ...
                <p className="mt-1 text-sm text-slate-500">Scheduled and confirmed appointments from CarePlus.</p>
                <div className="mt-6 overflow-hidden rounded-xl border border-slate-200">
                  {upcoming.length === 0 ? (
                    <div className="p-10 text-center text-sm text-slate-500">No upcoming appointments found.</div>
                  ) : (
                    <div className="divide-y divide-slate-100">
                      {upcoming.map((appointment) => (
                        <div key={appointment.appointment_id} className="flex flex-col gap-2 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <p className="text-sm font-medium text-slate-900">{patientMap.get(appointment.patient_id) ?? 'Patient'}</p>
                            <p className="mt-1 text-xs text-slate-500">{doctorMap.get(appointment.doctor_id) ?? 'Doctor'}</p>
                          </div>
                          <div className="text-left sm:text-right">
                            <p className="text-sm font-medium text-slate-800">{formatDate(appointment.appointment_date)}</p>
                            <p className="mt-1 text-xs text-slate-500">{formatTime(appointment.appointment_time)} · {appointment.appointment_status}</p>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <section className="rounded-2xl border border-[var(--border)] bg-white p-6">
                ...
                <div className="mt-5 space-y-4 text-sm">
                  {[
                    ...
                    ['Appointment automation', 'Active'],
                    ['Notification tracking', 'Active'],
                  ].map(([item, status]) => (
                    <div key={item} className="flex justify-between gap-4"><span className="text-slate-600">{item}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{status}</span></div>
                  ))}
                </div>
              </section>
            </div>
